...
\label{tab:nrms-ablation}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{nrms_ablation.png}
\caption{The three arms of Table~\ref{tab:nrms-ablation} drawn to a common
axis, with per-impression bootstrap error bars. The panels are on the same
scale, so the visible fact is that every arm sits well above the 0.5 chance
line while the widest gap between any two arms is 0.0072 AUC on EB-NeRD and
0.0135 on MIND, which is why the paired intervals of
Figure~\ref{fig:nrms-ci} rather than the point estimates carry the argument.}
\label{fig:nrms-ablation}
\end{figure}
...
only. Figure~\ref{fig:nrms-ci} shows all six paired comparisons at once, and
the colouring is the reading of the section, since MIND's
\code{baseline\,$\to$\,recency} interval is clear of zero on both splits while
EB-NeRD's is clear of zero on val alone.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{nrms_paired_ci.png}
\caption{Paired bootstrap 95\% intervals on the AUC difference for each
pairwise step of the ablation, on both splits. An interval drawn in blue
excludes zero and one drawn in red does not. The two middle rows of each panel
are what separates the padding mask from the recency signal, and they point in
...
not something the ablation design can separate.
